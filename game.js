/**
 * STUDY FARM SIMULATOR - GAME ENGINE (BULLETPROOF EDITION)
 */
...
// Global DOM Variables (Assigned after load)
...
// ==========================================
// 1. SYSTEM BOOTUP (PREVENTS CRASHES)
// ==========================================
window.addEventListener('DOMContentLoaded', () => {
    console.log("DOM loaded. Booting game engine...");
...
function updatePlayer() {
    if (!world || !playerEl) return; // Defensive check
    ...
    // CORRECT ASSET PATH (Direct in assets folder)
...
function checkInteractions() {
    ...
    if (currentInteractable && interactZones[currentInteractable].el) {
...
function rebuildEntities() {
    ...
    gameState.chickens.forEach(data => {
        ...
        el.style.backgroundImage = `url('assets/${data.type}.png')`; // CORRECT ASSET PATH
    ...
    gameState.eggs.forEach(egg => {
        ...
        el.style.backgroundImage = `url('assets/egg.png')`; // CORRECT ASSET PATH
...
function updateChickens() {
    activeChickens.forEach(chickenObj => {
        ...
        if (c.vx < 0) el.classList.add('facing-right'); 
...
function feedChickens() {
    ...
    activeChickens.forEach(obj => {
        const c = obj.data;
        c.state = 'eat';
        c.vx = (900 + (Math.random()*40-20) - c.x) / 60;
        c.vy = (1000 + (Math.random()*40-20) - c.y) / 60;
        c.timer = 180; 
        setTimeout(() => {
            showNotification(`❤️ ${c.name} is happy!`);
            c.happiness = 100; c.hunger = 100;
...
function buyChicken() {
    if (gameState.money >= 4000) {
        ...
        spawnNewChicken(); saveGame(); rebuildEntities(); closeModals();
    ...
    setTimeout(() => {
        if (gameState.dailyStudyMinutes < 30) {
            ...
            if (gameState.consecutiveMissedDays >= 2 && weakChickenIdx !== -1) {
                const dead = gameState.chickens.splice(weakChickenIdx, 1)[0];
                showNotification(`🪶 ${dead.name} waited for you today.`);
                gameState.consecutiveMissedDays = 1; 
            } else if (gameState.chickens.length > 0) {
                gameState.chickens[Math.floor(Math.random() * gameState.chickens.length)].isWeak = true;
                showNotification(`A chicken is weak because you didn't study.`);
            }
        } else {
            gameState.consecutiveMissedDays = 0;
            gameState.chickens.forEach(c => {
                c.isWeak = false; c.age++; 
                if (c.type === 'baby' && c.age > 3) c.type = 'white'; 
            });
        }
        ...
        if (gameState.eggs.length > 0) {
            ...
            gameState.eggs = [];
            for(let i=0; i<numHatched; i++) spawnNewChicken(true);
            showNotification(`🥚 ${numHatched} egg(s) hatched!`);
        }

        gameState.chickens.forEach(c => {
            if (!c.isWeak && c.type !== 'baby' && Math.random() < 0.2) { 
                gameState.eggs.push({ x: c.x, y: c.y });
                c.state = 'tired'; c.timer = 9999; 
            }
        });
